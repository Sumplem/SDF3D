...
bool updateActiveInputLinkDrag(SdfGraph& graph, const CanvasFrame& frame, const std::vector<GraphSocketAnchor>& anchors, bool& draggingInputLink, SdfGraphNodeId& dragInputNode, std::string& dragInputSocket, bool& releasedOnEmpty)
{
    ...
    if (const std::optional<ImVec2> to = findSocketAnchor(anchors, dragInputNode, dragInputSocket, false)) {
        const ImVec2 mouse = ImGui::GetIO().MousePos;
        const float handle = scaleValue(frame, 70.0f);
        frame.drawList->AddBezierCubic(mouse, {mouse.x + handle, mouse.y}, {to->x - handle, to->y}, *to, IM_COL32(255, 210, 110, 255), scaleValue(frame, 3.0f));
    }
